...
#TODO "cythonize" more
cpdef double p_spectrum(ng_a:List[int], ng_b:List[int], int n= 2) except? -1:
...
#TODO "cythonize" more
def sim_func_sim_list(tuples:List[Tuple[List[int], List[int]]], n_ngram: int , func: str) -> float:
    ...
    #TODO do this nicer
    if func == "p_spectrum":
        f = partial(p_spectrum, n=n_ngram)
    else: #ngram_sim
        f = partial(ngram_sim, n=n_ngram)
